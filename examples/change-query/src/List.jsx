...
import { stringifyParams, parseQuery } from 'url-search-utils';
...
export default reduxFilterlist({
  ...
  loadItems: ({
    sort,
    appliedFilters,
  }) => fetch(constructUrl('/cars', Object.assign({}, appliedFilters, {
  ...
  getStateFromProps: ({
    ...
    const {
      sort,
      ...parsedSearch
    } = parseQuery(search.substring(1, search.length), {
      page: 'number',
      perPage: 'number',
  ...
  onBeforeRequest: (listState, { history }, actionType) => {
    switch (actionType) {
      case 'loadItems':
      case 'loadItemsOnInit':
      case 'setStateFromProps':
        return;
